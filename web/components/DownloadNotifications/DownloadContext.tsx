...
export const DownloadProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (index !== -1) {
                            ...
                            const processedMessage = data.message !== undefined
    ...
    const addDownload = useCallback((download: Omit<DownloadProgress, 'progress'> & { progress?: number }) => {
        setDownloads(prev => {
                ...
                return prev.map(d => {
                    if (d.id === download.id) {
                        return {
                            ...
                            message: typeof download.message === 'string'
                                ? download.message
        ...
        setDownloads(prev =>
            prev.map(download => {
                if (download.id === id) {
                    ...
                    const updatedMessage = data.message !== undefined
