      ...
      setDatasetDetails((prevDetails) => {
        ...
        const updatedCategories = prevDetails.categories.map((category) => ({
          ...
          level1_clusters: category.level1_clusters.map((level1Cluster) =>
            level1Cluster.id === clusterId ? { ...level1Cluster, title: data.title } : level1Cluster
          ),
        }));
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ detail: 'Failed to fetch clusters' }));
      ...
      if (!data.categories || !Array.isArray(data.categories)) {
        console.warn(
          `No categories found or invalid format for dataset ${datasetId}, version ${selectedVersion}`
        );
        ...
        data.categories.forEach((category: CategoryResponse) => {
          category.level1_clusters.forEach((level1Cluster: Level1ClusterResponse) => {
            if (
              !level1Cluster.title &&
              level1Cluster.texts.length > 0 &&
              !titlingStatus[`cluster-${level1Cluster.id}`]
            ) {
    ...
    return (
      <>
        {paginatedLevel1Clusters.map((level1Cluster) => {
          ...
          const subclusterPercentage = calculatePercentage(
            level1Cluster.text_count,
            categoryTextCount
          );
            ...
            >
              <Text className={classes.subclusterTitle} lineClamp={2}>
                {titleStatus?.status === 'in_progress' ? (
                  ...
                  level1Cluster.title ||
                  titleStatus?.title ||
                  `Subcluster ${level1Cluster.l1_cluster_id}`
                )}
              </Text>
              <div>
                <Group align="flex-end" gap="xs">
                  <Text className={classes.value}>{level1Cluster.text_count.toLocaleString()}</Text>
                  <Text
            ...
            <Select
              ...
              onChange={(value) => {
                if (value) {
                  ...
                  router.replace(
                    `/dashboard/datasets/cluster?id=${datasetId}&version=${newVersion}`
                  );
                } else {
  ...
  return (
    <Container size="xl" py="xl">
      <div className={classes.root}>
        ...
        {!loading &&
          (!datasetDetails ||
            !datasetDetails.categories ||
            datasetDetails.categories.length === 0) && (
            <Alert color="blue" title="No Clusters Found">
              No cluster data available for Version {selectedVersion}. This could be due to an
              incomplete clustering process or no clusters being generated.
            </Alert>
          )}
